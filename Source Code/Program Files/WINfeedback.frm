...
Private Function CompileFeedback() As String
    ...
    CompileFeedback = CompileFeedback & "Product" & vbTab & vbTab & "- " & App.ProductName & " " & App.Major & "." & App.Minor & "." & App.Revision & vbNewLine
    CompileFeedback = CompileFeedback & "User" & vbTab & vbTab & "- "
    ...
    If Not ValidMail And Not ValidName Then
        CompileFeedback = CompileFeedback & "Anonymous"
    Else
        ...
        End If
    End If
    CompileFeedback = CompileFeedback & "Feedback Type" & vbTab & "- " & Mid$(lblType(TypeSel).Caption, 2) & vbNewLine & vbNewLine
    
    CompileFeedback = CompileFeedback & "USER'S COMMENT" & vbNewLine
    CompileFeedback = CompileFeedback & Chr$(34) & txtDesc.Text & Chr$(34) & vbNewLine & vbNewLine
    
    CompileFeedback = CompileFeedback & "RATINGS" & vbNewLine
    For TempNum = 0 To 4
        CompileFeedback = CompileFeedback & lblRatingType(TempNum).Caption & vbTab & IIf(TempNum = 4, vbTab, vbNullString) & "- " & Format$(RateVal(TempNum), "0.0") & vbNewLine
    Next
...
Public Function SendFeedback()
    ...
    Set FeedbackMsg = CreateObject("CDO.Message")
    FeedbackMsg.TextBody = CompileFeedback
    If ValidMail Then
        FeedbackMsg.To = txtMail.Text
        FeedbackMsg.BCC = "support@anico.in"
    Else
        FeedbackMsg.To = "support@anico.in"
    End If
    FeedbackMsg.From = """ANIco.in Feedback Management System"" <feedback@anico.in>"
    FeedbackMsg.Subject = "Feedback for " & App.ProductName
    If Len(Attachments) <> 0 Then
        For TempNum = 1 To Val(lblAttachCount.Caption)
            PrevOccur = InStr(PrevOccur + 1, Attachments, "*")
            FeedbackMsg.addattachment Mid$(Attachments, PrevOccur + 1, InStr(PrevOccur + 1, Attachments, "*") - PrevOccur - 1)
        Next
    End If
    FeedbackMsg.configuration.fields.Item("http://schemas.microsoft.com/cdo/configuration/sendusing") = 2
    FeedbackMsg.configuration.fields.Item("http://schemas.microsoft.com/cdo/configuration/smtpserver") = "smtp.anico.in"
    FeedbackMsg.configuration.fields.Item("http://schemas.microsoft.com/cdo/configuration/smtpserverport") = 25
    FeedbackMsg.configuration.fields.Item("http://schemas.microsoft.com/cdo/configuration/smtpauthenticate") = 1
    FeedbackMsg.configuration.fields.Item("http://schemas.microsoft.com/cdo/configuration/sendusername") = "feedback@anico.in"
    FeedbackMsg.configuration.fields.Item("http://schemas.microsoft.com/cdo/configuration/sendpassword") = "<AVAZ9C]r%Sp8=$lFww^>4MK"
    FeedbackMsg.configuration.fields.Update
    FeedbackMsg.Send
    cmdSend.Caption = "Sent"
    Unload Me
    Exit Function
Hell:
    cmdSend.Caption = "Failed"
    Enabled = True
